...
class MenuDelegate extends Ui.Menu2InputDelegate {
    ...
    // Menu2 option picker is fullmem-only - select gets an in-place list
    // editor on 64 KB widget devices instead (see EntityListController),
    // so the (:lowmem) variant below never references
    // MenuController.getSelectMenuEntityId(), itself :fullmem-only.
    // Hass.selectOption() is not :fullmem-only - both mechanisms call it.
    (:fullmem)
