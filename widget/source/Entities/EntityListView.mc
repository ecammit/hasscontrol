...
class EntityListView extends Ui.View {
  ...
  hidden function getIconDrawable(entity) {
    var mdiDrawable = Utils.getMdiIconDrawable(entity);
    if (mdiDrawable != null) {
      return mdiDrawable;
    }
  ...
  // Null-safe string equality helper for the icon cache. Monkey C's != on
  // Strings compares references, not content, so use .equals() when both are
  // non-null and treat null/!null as different.
  hidden function strEq(a, b) {
